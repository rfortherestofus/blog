...
-   **Subtitle**: regular face, medium sized, left-aligned to plot,
    light gray text
-   **Caption**: regular face, smallest sized, left-aligned to plot,
    dark gray text
...
Below, you can see how we use these arguments to customize various elements of our theme.
...
While we created `theme_nyt()` specifically for a
...
should show grid lines for the x-axis and y-axis. First, we'll create a scatterplot.
...
Next, we can apply the them to our plot, setting `gridline_x` and `gridline_y` to `TRUE`.
