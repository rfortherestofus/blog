...
In our [consulting work](https://rfortherestofus.com/consulting) at R for the Rest of Us, we create tables all the time.
We don't just make plain tables, we make tables that look good and communicate well. 
This is important because:

1.  A clean design is more convincing
2.  A powerful design stands out so that people actually look at our tables
3.  We can use colors to reflect our clients' branding

We recently helped the [Oregon Voices project](https://orvoices.org/your-county/) to create customized data reports for every county in Oregon.
...
Designing a table and adding a corporate identity to the design can mean lots of time-consuming work.
...
Now, let's format the column labels (with the `cols_label()` function) and add a title and subtitle (with the `tab_header()` function) to describe our data.
...
Next, let's change the title's font and color.
...
In principle, we can just drop all of the code into a [reusable function](https://book.rwithoutstatistics.com/packages-chapter.html#creating-your-own-functions).
...
This is an elegant way to make your tables more appealing.
And if you're looking for a bit of inspiration for your tables, you can always check out the [themes from the `gtExtras` package](https://jthomasmock.github.io/gtExtras/reference/index.html).
