...
Adding the `rect()` function with `footer` gives us, not surprisngly, a rectangle. We set its width, height, and the `outset` argument, which ensures that the blue rectangle goes all the way to the horizontal edge. The rectangle's color is set with `fill: rgb("#68ACE5")` (in order to use hex values in Typst, you have to wrap them in the `rgb()` function).
...
TODO: Add source function here as intermediate complicated step?
